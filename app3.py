...
def cross_validate_person(
    ...
    queries = [
        ...
        f'"{name}" "{company}" present',
        f'"{name}" "{company}" "{persona}" -former -previously -ex',
    ]
    ...
    return {
        ...
        "Verification Status": (
            ...
            if (
                ...
                and not analysis["reason_details"]
            )
            else (
                "REVIEW"
                if (
                    analysis["candidate_company_score"] >= 90
                    and analysis["candidate_title_score"] >= 70
                    and analysis["negative_evidence_count"] == 0
                    and (
                        analysis["company_evidence_score"] > 0
                        or analysis["role_evidence_score"] > 0
                    )
                )
                else "REJECTED"
            )
...
def run_persona_pipeline(
    ...
    # Same LinkedIn profile can appear under several closely related
    # personas. Keep the strongest verification for the profile/persona.
